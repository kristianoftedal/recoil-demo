...
import { winePropList } from './wineProps';
import api from 'api';
import {
  acidRange,
  alcoholRange,
...
export const allWines = selector({
  key: 'allWines',
  get: async ({ get }) => await api.getWines(get(pageSize))
})

export const wines = atom({
  key: 'wines',
  default: allWines
})
...
export const transformedWineList = selector({
  ...
  get: ({ get }) => {
    ...
    const transformed = transform(wineList, arr =>
      arr
